...
def sza_calculator_v2_exact(dset, sza_threshold=88.85, sza_nu=0.968):
    ...
    #Setting all night values to a small number so its multplying by the max (determined by nu) at sunset
    cos_zen_grid[cos_zen_grid<=0] = 1e-4
    ...
    #Applying the solar zenith angle adjustment to the ABI reflectance factor data
    cmi = dset['CMI'][:] #Reflectance factor
    sza_cmi = np.clip(cmi / cos_zen_grid, a_min=None, a_max=0.12999) #Clipping so we aren't going over the max value in AWIPS
...
def realtime_slider_ch02(data_loc, slot, output_loc, time_string):
    ...
    width_px = 5000 #dset.dimensions['x'].size
    height_px = 3000 #dset.dimensions['y'].size
...
def satpy_plotter_slider_v1(red, green, blue, output_loc,):
    ...
    scn.save_dataset('C02', writer='simple_image', filename=output_loc+"{platform_shortname}_SZA_{name}_s{start_time:%Y%j%H%M}.png")
    ...
    #Saving out the RGB
    scn.save_dataset('day_cloud_type_distinction_raw', writer='simple_image', filename=output_loc+"{platform_shortname}_SZA_{name}_s{start_time:%Y%j%H%M}.png")
