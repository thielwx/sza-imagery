...
temp_file_list  = sza.aws_downloader(slots, channels, dload_time, dload_time)
...
temp_file_rgb_list  = sza.aws_downloader(slots, rgb_extra_channels, dload_time, dload_time)
...
for slot in slots:
    #Red 
    pattern = '*C13+'+slot+'_'+file_time
    r_files.extend([temp_file_loc+item for item in temp_file_rgb_list if fnmatch.fnmatch(item, pattern)])
    #Green
    pattern = '*C02+'+slot+'_'+file_time
    g_files.extend([output_loc+item for item in realtime_files if fnmatch.fnmatch(item, pattern)])
    #Blue
    pattern = '*C05+'+slot+'_'+file_time
    g=b_files.extend([output_loc+item for item in realtime_files if fnmatch.fnmatch(item, pattern)])
...
def realtime_image_driver(red, green, blue, output_loc, slot, time_string):
    ...
    #Plotting the rgbs
    sza.rgb_plotter(rgb, slot, output_loc, time_string)
